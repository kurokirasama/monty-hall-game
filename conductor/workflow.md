...
2.  **Automated Manual Verification (Mandatory):**
    - Evaluate if the required verification steps are "simple" (Read-Only, Non-Destructive, Fast Execution).
    - If they meet these criteria, **execute them autonomously** (e.g., checking file contents, running scripts, analyzing logs).
    - If automated verification fails, attempt an **Auto-Fix & Retry once**.
    - If retry fails, halt and ask the user for manual intervention.
    - All automated verification outcomes must be documented in task/phase summaries and git notes.
...
Whenever an `ask_user` tool call is about to be made, a Discord notification MUST be sent first.
- **Content:** Exact question, Task Metadata (Track ID, Phase, Task), and Context (modified files, summary, simplified diff).
- **Command:** Execute via Nushell `evaluate` tool: `to-discord $message -p`.
- **Timing:** Immediately before calling `ask_user`.

### Discord Notification for Long Tasks (5min+)
If a task takes 5 minutes or more:
1.  **Draft Report (English):** Concise summary, next steps, and status (waiting for user or continuing automatically).
2.  **Send via Discord:** Execute via Nushell `evaluate` tool: `to-discord $message -p`.
3.  **Sequence Priority:** If the next step is `ask_user`, send the Discord notification first.
